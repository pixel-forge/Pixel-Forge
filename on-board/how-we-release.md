...
Changelogs are generated by `@changesets/changelog-git`, which records the commit
hash alongside your summary. It needs no credentials, which suits releasing from a
laptop.

If this repo later takes contributions through pull requests, switching to
`@changesets/changelog-github` gives richer entries with PR and author links. It
requires both a `repo` option in `.changeset/config.json` and a `GITHUB_TOKEN` in
the environment on every `changeset version` run — omitting either is a hard
failure, not a degraded changelog.
